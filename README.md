...
> ## 注意
>
> 本リポジトリは、OEM firmware と OpenWrt を本体のモード切り替えスイッチで
> 選択するデュアルブート環境を、WSR-2533DHP2で実現することを目的に開発・検証した
> 研究資産です。現在の実機では、ROUTER=OEM、AP=OpenWrt、WB=Hybrid Selector Console
> のデュアルブート構成に完全に成功しています。ただし、現時点ではCodexによる概要整理を
> 経て公開した段階であり、完成された汎用製品やワンクリック再現手順ではありません。
>
> NANDのBad Block位置など個体差によっては同じ構成を再現できない可能性があり、その他にも
> [既知の制約](docs/09-results.md)があります。最終的には、殻割りを必要とせずに再現できる
> 手順とHybrid Selectorを完成させ、誰でも追試できる成果物として公開することを目指して
> います。ただし、その実現時期や実現可否は未定です。
